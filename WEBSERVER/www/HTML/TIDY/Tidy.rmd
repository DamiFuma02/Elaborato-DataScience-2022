...
# View(amazon)
spec(amazon)
colnames(amazon)[8] = "Year"
colnames(amazon)[11] = "Genres"

amazon = amazon %>% filter(!is.na(type))  %>% select(-date_added,-description,-show_id) %>% mutate(Year = as.integer(Year)) 
...
```{r}
colnames(disney)[8] = "Year"
colnames(disney)[11] = "Genres"
disney = disney %>% select(-description,-date_added,-show_id) %>% mutate(Year = as.integer(Year))
...
disneyRatingTypes = data.frame(
  ...
  rating = disRats,
  movie = (disRats %in% levels(factor(disneyMovies$rating)))
)
```
...
```{r}
colnames(HighestGrossers)[11] = "ageRating"
...
```{r}
levels(factor(HighestGrossers$ageRating))
licenseTable = data.frame(
  license = c("G","PG","PG-13","R","NA"),
  maxYear = c(9,12,17,99,NA)
)
```
...
spec(netflix)
colnames(netflix)[8] = "Year"
colnames(netflix)[11] = "Genres"
netflix = netflix %>% select(-date_added,-description) %>% mutate(Year = as.integer(Year))
...
(netflixRatingTypes = data.frame(
  ...
  rating = netRat,
  movie = (netRat %in% levels(factor(netflixMovies$rating)))
))
```
...
if (all(awards$year_ceremony == awards$year_film+1)){
  ...
  awards = awards %>% select(-year_ceremony) %>% mutate(year = as.integer(year))
}
...
```{r}
spec(sequels)
sequels = sequels %>% select(-`Release Date`,-Description,-URL) %>% mutate(Year = as.integer(Year ))
```
...
THE DATA FORMAT IS CHOSEN [HERE](./FileDim.html#fileType)
...
saveRDS(amazon,paste(amazonPath,"amazon.rds",sep="") )
colnames(amazonMovies)  = tolower(colnames(amazonMovies))
saveRDS(amazonMovies,paste(amazonPath,"amazonMovies.rds",sep="")  )
colnames(amazonShows)  = tolower(colnames(amazonShows))
saveRDS(amazonShows,paste(amazonPath,"amazonShows.rds",sep="")  )
saveRDS(amazonGenres,paste(amazonPath,"amazonGenres.rds",sep="") )
saveRDS(amazonRatingTypes,paste(amazonPath,"amazonRatingTypes.rds",sep="") )
saveRDS(amazonCountries,paste(amazonPath,"amazonCountries.rds",sep="") )
saveRDS(amazonDirectors,paste(amazonPath,"amazonDirectors.rds",sep="") )
...
saveRDS(disney,paste(disneyPath,"disney.rds",sep="")  )
colnames(disneyMovies) = tolower(colnames(disneyMovies) )
saveRDS(disneyMovies,paste(disneyPath,"disneyMovies.rds",sep="")  )
colnames(disneyShows) = tolower(colnames(disneyShows))
saveRDS(disneyShows,paste(disneyPath,"disneyShows.rds",sep="")  )
saveRDS(disneyGenreTable,paste(disneyPath,"disneyGenreTable.rds",sep="") )
saveRDS(disneyRatingTypes,paste(disneyPath,"disneyRatingTypes.rds",sep="") )
saveRDS(disneyCountries,paste(disneyPath,"disneyCountries.rds",sep="") )
```
...
saveRDS(HighestGrossers,paste(grossPath,"gross.rds",sep="")  )
saveRDS(grossGenreTable,paste(grossPath,"grossGenreTable.rds",sep="") )
saveRDS(grossDistributors,paste(grossPath,"grossDistributors.rds",sep="") )

saveRDS(licenseTable,paste(grossPath,"licenseTable.rds",sep="") )
...
saveRDS(movies,paste(movielensPath,"movies.rds",sep="")  )
colnames(ratings) = tolower(colnames(ratings))
saveRDS(ratings,paste(movielensPath,"ratings.rds",sep="")  )
users = users %>% mutate(Male = ifelse(Male,1L,0L))
colnames(users) = tolower(colnames(users))
saveRDS(users,paste(movielensPath,"users.rds",sep="") )
saveRDS(workTable,paste(movielensPath,"workTable.rds",sep="") )
saveRDS(ageTable,paste(movielensPath,"ageTable.rds",sep="") )
saveRDS(movieLensGenreTable,paste(movielensPath,"movieLensGenreTable.rds",sep="") )
saveRDS(uszips,paste(movielensPath,"uszips.rds",sep="") )
```
...
saveRDS(netflix,paste(netflixPath,"netflix.rds",sep="") )
colnames(netflixMovies) = tolower(colnames(netflixMovies))
saveRDS(netflixMovies,paste(netflixPath,"netflixMovies.rds",sep="")  )
colnames(netflixShows) = tolower(colnames(netflixShows))
saveRDS(netflixShows,paste(netflixPath,"netflixShows.rds",sep="") )
saveRDS(netflixGenres,paste(netflixPath,"netflixGenres.rds",sep="") )
saveRDS(netflixRatingTypes,paste(netflixPath,"netflixRatingTypes.rds",sep="") )
saveRDS(netflixCountries,paste(netflixPath,"netflixCountries.rds",sep="") )
...
saveRDS(awards,paste(oscarPath,"oscars.rds",sep="") )
...
saveRDS(sequels,paste(sequelPath,"sequels.rds",sep="")  )
saveRDS(seqGenres,paste(sequelPath,"sequelGenres.rds",sep=""))
